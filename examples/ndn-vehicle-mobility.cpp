...
//     cmd.AddValue("_transmissionInterval", "Advertisement Packet Transmission Frequency: ", _transmissionInterval);
  ...
  consumerHelper.SetPrefix("/RPP/"+applicationType+"/"+contentType+"/"); //// setting up naming scheme for consumer to request 
  consumerHelper.Install(mobileNodes);
  consumerHelper.Install(mobileNodes.Get(3));

  ndn::AppHelper producerHelper("ns3::ndn::Producer");
  producerHelper.SetPrefix("/RPP/"+applicationType+"/"+contentType+"/"); //// setting up naming scheme for producer to response
