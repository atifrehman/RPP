...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  // Atif-Code: Start
  shared_ptr<lp::SpeedTag> speedTag = netPkt.getTag<lp::SpeedTag>();
  if (speedTag != nullptr) {
    lpPacket.add<lp::SpeedTagField>(*speedTag);
  }
  else {
    lpPacket.add<lp::SpeedTagField>(0);
  }

 shared_ptr<lp::PriorityTag> priorityTag = netPkt.getTag<lp::PriorityTag>();
  if (priorityTag != nullptr) {
    lpPacket.add<lp::PriorityTagField>(*priorityTag);
  }
  else {
    lpPacket.add<lp::PriorityTagField>(0);
  }


   shared_ptr<lp::SINRTag> sinrTag = netPkt.getTag<lp::SINRTag>();
  if (sinrTag != nullptr) {
    lpPacket.add<lp::SINRTagField>(*sinrTag);
  }
  else {
    lpPacket.add<lp::SINRTagField>(0);
  }
...
//Atif-Code: Start 
// decoding tags to forward

  if (firstPkt.has<lp::SpeedTagField>()) {
    interest->setTag(make_shared<lp::SpeedTag>(firstPkt.get<lp::SpeedTagField>()));
  }
  else
  {
    std::cout<<"Speed Tag is missing in generic link service.";
  }

  if (firstPkt.has<lp::PriorityTagField>()) {
    interest->setTag(make_shared<lp::PriorityTag>(firstPkt.get<lp::PriorityTagField>()));
  }
  else
  {
    std::cout<<"Priority Tag is missing in generic link service.";
  }

  if (firstPkt.has<lp::SINRTagField>()) {
    interest->setTag(make_shared<lp::SINRTag>(firstPkt.get<lp::SINRTagField>()));
  }
  else
  {
    std::cout<<"SINR Tag is missing in generic link service.";
  }
...
if (firstPkt.has<lp::GeoTagField>()) {
  ...
  {
    std::cout<<"Geo Tag is missing in generic link service.";
  }
